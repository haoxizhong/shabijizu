...
int instr_limit = 10000000; /* Instruction limit [TTY only] (-l) */
...
void do_if_stage()
{
	...
	/* Ready to fetch instruction.  Speculatively fetch register byte
	   and immediate word
	 */
	imem_error = !get_byte_val(mem, valp+firstp, &instr);
	imem_icode = HI4(instr);
	imem_ifun = LO4(instr);
	if (!imem_error) {
		byte_t junk;
		/* Make sure can read maximum length instruction */
		imem_error = !get_byte_val(mem, valp+5+firstp, &junk);
	}
...
//		sim_log("\tFetch: Instruction code 0x%x invalid\n", instr);
	...
	if (gen_need_valC()) {
		get_word_val(mem, valp+firstp, &valc);
		valp+= 4;
	}
